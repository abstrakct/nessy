...
typedef ImU8 (*ReaderFunctionPointer)(const ImU8 *, size_t);
typedef void (*WriterFunctionPointer)(ImU8 *, size_t, ImU8);
...
struct MemoryEditor {
    ...
    // ImU8 (*ReadFn)(const ImU8 *data, size_t off);      // = 0      // optional handler to read bytes.
    // void (*WriteFn)(ImU8 *data, size_t off, ImU8 d);   // = 0      // optional handler to write bytes.
    ReaderFunctionPointer ReadFn;
    WriterFunctionPointer WriteFn;
    ...
    // void SetReadFn(ReaderFunctionPointer f)
    // {
    //     ReadFn = f;
    // }

    void AttachMachine(std::shared_ptr<Machine> m)
    {
        nes = m;
    }
        ...
        for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
        {
            ...
            for (int n = 0; n < Cols && addr < mem_size; n++, addr++) {
                ...
                if (DataEditingAddr == addr) {
                    ...
                    if (DataEditingTakeFocus) {
                        ...
                        sprintf(DataInputBuf, format_byte, nes->editorCpuRead(mem_data, addr) /*this->ReadFn ? ReadFn(mem_data, addr) : mem_data[addr]*/);
                    }
                    ...
                    sprintf(user_data.CurrentBufOverwrite, format_byte, nes->editorCpuRead(mem_data, addr) /*ReadFn ? ReadFn(mem_data, addr) : mem_data[addr]*/);
                    ...
                    if (data_write && sscanf(DataInputBuf, "%X", &data_input_value) == 1) {
                        // if (WriteFn)
                        nes->editorCpuWrite(mem_data, addr, (ImU8)data_input_value); //WriteFn(mem_data, addr, (ImU8)data_input_value);
                        // else
                    ...
                    ImU8 b = nes->editorCpuRead(mem_data, addr); /* ReadFn ? ReadFn(mem_data, addr) : mem_data[addr]*/
            ...
            if (OptShowAscii) {
                ...
                for (int n = 0; n < Cols && addr < mem_size; n++, addr++) {
                    ...
                    unsigned char c = nes->editorCpuRead(mem_data, addr); //ReadFn ? ReadFn(mem_data, addr) : mem_data[addr];
    ...
    void DrawPreviewData(size_t addr, const ImU8 *mem_data, size_t mem_size, ImGuiDataType data_type, DataFormat data_format, char *out_buf, size_t out_buf_size) const
    {
        ...
        for (int i = 0, n = (int)size; i < n; ++i)
            buf[i] = nes->editorCpuRead(mem_data, addr + i);
        // else
